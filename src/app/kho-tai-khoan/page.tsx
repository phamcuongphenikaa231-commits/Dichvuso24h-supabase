...
export default function KhoTaiKhoanPage() {
  const [categoryMeta, setCategoryMeta] = useState<CategoryMeta | undefined>(() =>
    productService.getCategoriesFlat().find((c) => c.slug === 'kho-tai-khoan')
  );

  useEffect(() => {
    const refresh = () => {
      setCategoryMeta(productService.getCategoriesFlat().find((c) => c.slug === 'kho-tai-khoan'));
    };
    refresh();
