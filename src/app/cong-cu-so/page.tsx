...
export default function CongCuSoPage() {
  const [categoryMeta, setCategoryMeta] = useState<CategoryMeta | undefined>(() =>
    productService.getCategoriesFlat().find((c) => c.slug === 'cong-cu-so')
  );

  useEffect(() => {
    const refresh = () => {
      setCategoryMeta(productService.getCategoriesFlat().find((c) => c.slug === 'cong-cu-so'));
    };
    refresh();
